...
func (f *Fn_) call(
	...
	if missingIn := findMissing(dat0, f.inputs); len(missingIn) > 0 {
		return OfErr(), hippoerr.NewFnMissingKeysError(missingIn...)
	}
	...
	if missingOut := findMissing(ret4, f.outputs); len(missingOut) > 0 {
		return OfErr(), hippoerr.NewFnMissingOutputError(missingOut...)
	}
